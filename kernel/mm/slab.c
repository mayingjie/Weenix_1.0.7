...
struct slab {
		...
		/*MC means the address to the free page size requried allocate memory
		 s_free = s_addr  after each time _slab_allocator_grow
		 each time slab_obj_alloc is called a object is removed, s_free is used to track back to the origiranl slab
*/
...
struct slab_allocator {
		...
		ssize_t: signed int 32 bit
		defiened in kernel/include/types.h
		 sa_objize : store the size of allocator, depends on its type */
...
struct slab_bufctl {
		/*MC 
		 union
		 before allocation, sb_next point ot the next availabel obje
		 after being removed, using sb_slab to record the orgiinal salb where it is*/
        union {
...
/* mc
 link to all  allocatorm, also include thread */
static struct slab_allocator *slab_allocators = NULL;

/* MC
 slab_init() will initial it
 ? when to call it , in kmain.c*/
...
_calc_slab_size(struct slab_allocator *allocator)
{
		...
		/* MC
		 each kinp of slab allocator has its own objsize requrued */
        /* Find the minimum page block size that this slab allocator requires. */
...
slab_allocator_create(const char *name, size_t size) {
		...
		/* MC
		 sla_allocator_allocator // a global one
		 static global struct head, type is also slab_allocator*/
...
_slab_allocator_grow(struct slab_allocator *allocator)
{
		...
		/* MC
		 addr  return the address of requested page size memory
		 defined in kernel/mm/page.c*/
		...
		#define obj_bufctl(allocator, obj) \
         ( (struct slab_bufctl*)(((uintptr_t)(obj)) + (allocator)->sa_objsize) )
		 uintptr_t  unsigned int or unisgned long
		 deifend in kernel/include/types.h
		 form all objects belonging to the same slab allocator in one list
		 and all set free, in pages gained in the same function */
...
slab_obj_alloc(struct slab_allocator *allocator)
{
        ...
        for (;;) {
				...
				/* MC
				 check if current slabs is use and is usedup by allocation
				 go through to find a a slb */
                while (slab && (slab->s_inuse == allocator->sa_slab_nobjs))
                        slab = slab->s_next;
				/* to chekc slb if inuse < numer of jbjects maximim limitation */
                if (slab && (slab->s_inuse < allocator->sa_slab_nobjs))
                        break;
				/* to increate slab if there is not any slab
				 if grow fail, return NULL */
...
slab_init()
{
		...
		/*MC
		  only "initia"  the space for slab_allocator(glonal variable), sa_slabs slab list still null
		 allocate in function  after going through there is no slabs
		 slab_obj_alloc(struct slab_allocator *allocator)
		
		last argument : calcuate objs size(frmo argu two + struct slab_bufctl size+ struct slab) can have */
         ...
         */
		/*MC 
		 order:from 6  to 18, size going through from 2^6 => 2^18 
		 cs: 13 units, 0~12
		 kmalloc_allocator_names: going through from  0 ~12 */
        cs = kmalloc_allocators;
        for (order = KMALLOC_SIZE_MIN_ORDER; order <= KMALLOC_SIZE_MAX_ORDER; order++, cs++) {
